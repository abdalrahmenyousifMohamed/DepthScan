...
functionary_model = "hivata/functionary-small-v3.2-AWQ"
Medius_model = "AMead10/SuperNova-Medius-AWQ"
supported_providers = ["openai", "deepseek", "openrouter", "gemini", "anthropic", "perplexity", "huggingface", "local",functionary_model,Medius_model]
...
if Custom:
    provider_mapping[functionary_model] = {
        ...
        "base_url": "https://z9v53bple9owlo-8010.proxy.runpod.net/v1",
        "api_key": "my-secret-api-key",
    }
    provider_mapping[Medius_model] = {
        "model": OpenAIChatCompletionsModel,  # or OpenAIResponsesModel based on your model type
        "base_url": "https://z9v53bple9owlo-8000.proxy.runpod.net/v1",
        "api_key": "my-secret-api-key",
    }
if OPENAI_API_KEY:
...
def create_default_config() -> LLMConfig:
    ...
    return  LLMConfig(
        search_provider=SEARCH_PROVIDER,
        reasoning_model_provider=functionary_model,
        reasoning_model=functionary_model,
        main_model_provider=Medius_model,
        main_model=Medius_model,
        fast_model_provider=functionary_model,
        fast_model=functionary_model,
    )
...
def model_supports_structured_output(model: Union[OpenAIChatCompletionsModel, OpenAIResponsesModel]) -> bool:
    ...
    res = any(provider in get_base_url(model) for provider in structured_output_providers)
    print(f"Model: {model}, Base URL: {get_base_url(model)} , res: {res}")
